...
const AdminCategories = () => {
  ...
  const reorderMutation = useMutation({
    mutationFn: (list) => api.put('/categories/reorder', { items: list.map((c, i) => ({ id: c._id, order: i })) }),
    onSuccess: () => { invalidate(); setLocalCats(null); },
    onError: (e) => { alert(e.message || 'Failed to save order'); setLocalCats(null); },
  });
  ...
  const displayTop = localCats ?? topLevel;
  ...
  // ── Drag-to-reorder ──────────────────────────────────────────────────────────
  const handleDragStart = (i) => { setDragIndex(i); setLocalCats([...displayTop]); };
  const handleDragEnter = (i) => { if (i !== dragIndex) setOverIndex(i); };
  const handleDrop = () => {
    if (dragIndex === null || overIndex === null || dragIndex === overIndex) {
      setDragIndex(null); setOverIndex(null); setLocalCats(null);
      return;
    }
    const reordered = [...displayTop];
                  ...
                  return (
                    <Fragment key={cat._id}>
                      ...
                      <tr
                        ...
                        className={[
                          'bg-[#f7faf7] border-b border-[#e8eee8] transition-colors',
                          !cat.isActive ? 'opacity-50' : '',
                          dragIndex === idx ? 'opacity-40 bg-[#f0f0f0]' : '',
                          overIndex === idx && dragIndex !== idx ? 'border-t-2 border-t-brand-green' : '',
                        ].join(' ')}
                      >
                        {/* Drag handle */}
                        <td className="px-4 py-3 w-12">
                          <div className="flex items-center gap-1 text-[#bbb] cursor-grab active:cursor-grabbing">
                            <GripVertical size={16} />
                            <span className="font-lato text-[11px] text-[#ccc]">#{idx + 1}</span>
                          </div>
                        </td>
                        ...
                        <td className="px-4 py-3">
                          <button onClick={() => toggleMutation.mutate(cat)} className="flex items-center gap-2 cursor-pointer">
                      ...
                      {isOpen && subs.map((sub) => (
                        <tr key={sub._id} className={`bg-white border-b border-[#f0f0f0] ${!sub.isActive ? 'opacity-50' : ''}`}>
                          ...
                          <td className="px-4 py-2.5">
                            <button onClick={() => toggleMutation.mutate(sub)} className="flex items-center gap-2 cursor-pointer">
